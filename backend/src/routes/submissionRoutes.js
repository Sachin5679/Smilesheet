...
router.post("/upload", protect, authorize("patient"), upload.single("image"), async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Submission uploaded successfully", submission: s });
  } catch (err) {
    console.error(err);
